...
class Hood : SubsystemBase() {
        ...
        TalonFXConfiguration().apply {
            MotorOutput =
                MotorOutputConfigs().apply { // Motor's default actions
                    ...
                    Inverted =
                        InvertedValue
                            .CounterClockwise_Positive // Motor's positive angle is against
                    // clockwise
                }
            CurrentLimits =
    ...
    private val positionRequest: PositionVoltage =
        PositionVoltage(0.0) // Motor's final position when angle = 0.0
    ...
    fun setAngle(angle: Angle) {
        ...
        motor.setControl(
            positionRequest.withPosition(angle[degrees])
        ) // Motor moves angle[degrees]
    }
